...
class InpatientRepository(BaseRepository):
    ...
    def check_room_availability(self, room_id):
        ...
        result = self.fetch_one(query, (room_id,))
        return result['available_beds'] if result else 0
    
    def create_admission(self, admission_data):
        ...
        new_admission = self.fetch_one(query_admission, params_admission)

        # 2. Update room availability: decrease available_beds by 1 using fetch_one to avoid missing attribute errors
        query_room_update = """
            UPDATE rooms 
        ...
        """
        self.fetch_one(query_room_update, (admission_data['room_id'],))
